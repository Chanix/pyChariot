...
<span class="line"><span>        |</span></span>
<span class="line"><span>        \\---appchips            # 目录：小应用包目录</span></span></code></pre></div><p>在这些目录下，再按照应用标识建立子目录，存放相应文件。</p><p>例如，有个项目，项目标识为 <code>{appchip_id}</code>，则：</p><ul><li>应用的应用包文件目录： <code>PYCHARIOT_HOME/appchips/{appchip_id}</code>；</li><li>应用的临时文件目录： <code>PYCHARIOT_HOME/temp/{appchip_id}</code>；</li><li>应用的数据文件目录： <code>PYCHARIOT_HOME/data/{appchip_id}</code>；</li><li>应用的浏览器组件数据目录： <code>PYCHARIOT_HOME/data/{appchip_id}</code>；</li></ul><p>这样约定的是目的是为了更便捷和统一的管理应用的各种文件。使用中，请按照这个约定来存储对应的文件。</p><h2 id="pychariot-home-appchips" tabindex="-1">PYCHARIOT_HOME/appchips <a class="header-anchor" href="#pychariot-home-appchips" aria-label="Permalink to &quot;PYCHARIOT_HOME/appchips&quot;">​</a></h2><p>运行于 pyChariot 上的应用，称为小应用 <code>appchip</code>。 所有小应用的程序包文件皆存放在 appchips 下，以小应用的标识作为其存放目录名称，其结构如下：</p><div class="language-text vp-adaptive-theme"><button title="Copy Code" class="copy"></button><span class="lang">text</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span>    appchip_id                  # 目录名即小应用的标识，请尽量使用 ANSI 字符（英语+数字）。</span></span>
...
<span class="line"><span>                                #   本地 httpd 服务器将以本目录作为 / 。</span></span></code></pre></div><ul><li>js/inject.js 主窗口打开后执行该脚本文件，运行此脚本时，扩展功能模块和常用变量已就绪，可以正常使用；</li><li>hotkeys 目录下存放系统级热键定义，以热键的按键为文件名，当热键被按下时，在主窗口内执行其中的脚本代码；</li></ul><p><em><br>嗯……本来想叫 “appclip”的，打错了……不过无论是 “薯片”还是 “芯片”，我都喜欢，就叫 <code>appchip</code> 吧 😃</em></p><h2 id="pychariot-home-wvdata" tabindex="-1">PYCHARIOT_HOME/wvdata <a class="header-anchor" href="#pychariot-home-wvdata" aria-label="Permalink to &quot;PYCHARIOT_HOME/wvdata&quot;">​</a></h2><p>浏览器组件数据存放目录。</p><p>浏览器组件数据和系统浏览器数据是隔离的，因此不会数据相互影响（例如登录数据，Cookie数据），以此最大可能的保证独立性。</p><p>因此，应用第一次运行时会略微慢一些（需要生成和保存这些数据）。同样基于此，不建议将 pyChariot 保存在劣质 U 盘中，因其读写速度与可靠性甚至差于移动机械硬盘。</p>`,21)]))}const C=s(i,[["render",l]]);export{_ as __pageData,C as default};
